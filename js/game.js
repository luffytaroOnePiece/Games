...
function renderGameMeta(game) {
  ...
  const ownerType = game.type || game.type;
  badgesEl.innerHTML = `
    <div class="detail-badges-row">
      ...
      ${
        ownerType === "Purchased"
          ? `<span class="card-ownership-badge badge-bought">🛒 Purchased${game.Price ? ` · ₹${game.Price}` : ""}</span>`
...
function updateLightboxImage() {
  ...
  if (lb.download) {
    ...
    lb.download.download =
      item.alt.split("/").pop() || `screenshot-${lb.current + 1}.jpg`;
  }
